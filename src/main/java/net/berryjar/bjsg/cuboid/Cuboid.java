...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
    ...
    public Cuboid(String worldName, String regionID, double x1, double y1, double z1, double x2, double y2, double z2) {

        this.worldName = worldName;
        this.x1 = Math.min((int)x1, (int)x2);
        this.x2 = Math.max((int)x1, (int)x2);
        this.y1 = Math.min((int)y1, (int)y2);
        this.y2 = Math.max((int)y1, (int)y2);
        this.z1 = Math.min((int)z1, (int)z2);
        this.z2 = Math.max((int)z1, (int)z2);
        this.regionID = regionID;

    }
    public Cuboid(String regionID, Location l1, Location l2) {
     ...
     */
    public Cuboid(String regionID, Location l1) {
        this(regionID, l1, l1);
    }
    ...
    public Cuboid(Cuboid other) {
        this(other.regionID, other.getWorld().getName(), other.x1, other.y1, other.z1, other.x2, other.y2, other.z2);
    }
     ...
     */
    public Cuboid(String regionID, World world, int x1, int y1, int z1, int x2, int y2, int z2) {
     ...
     * @param z2 - Z co-ordinate of corner 2
     */
    public Cuboid(String regionID, String worldName, int x1, int y1, int z1, int x2, int y2, int z2) {
        this.worldName = worldName;
        this.regionID = regionID;
        this.x1 = Math.min(x1, x2);
        this.x2 = Math.max(x1, x2);
     ...
     */
    public Cuboid(Map<String, Object> map, String regionID) {
    ...
    public Cuboid expand(CuboidDirection dir, int amount) {
        switch (dir) {
            case North:
                return new Cuboid(this.regionID, this.worldName, this.x1 - amount, this.y1, this.z1, this.x2, this.y2, this.z2);
            case South:
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1, this.z1, this.x2 + amount, this.y2, this.z2);
            case East:
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1, this.z1 - amount, this.x2, this.y2, this.z2);
            case West:
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1, this.z1, this.x2, this.y2, this.z2 + amount);
            case Down:
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1 - amount, this.z1, this.x2, this.y2, this.z2);
            case Up:
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1, this.z1, this.x2, this.y2 + amount, this.z2);
            default:
    ...
    public boolean contains(Player p) {
        if (!this.worldName.equals(p.getWorld().getName())) return false;
        return this.contains(p.getLocation().getBlockX(), p.getLocation().getBlockY(), p.getLocation().getBlockZ());
    }
    ...
    public Cuboid contract(CuboidDirection dir) {
        ...
        switch (dir) {
            case Down:
                ...
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1, this.z1, this.x2, face.getUpperY(), this.z2);
            case Up:
                while (face.containsOnly(Material.AIR) && face.getUpperY() < this.getUpperY()) {
                    face = face.shift(CuboidDirection.Up, 1);
                }
                return new Cuboid(this.regionID, this.worldName, this.x1, face.getLowerY(), this.z1, this.x2, this.y2, this.z2);
            case North:
                while (face.containsOnly(Material.AIR) && face.getLowerX() > this.getLowerX()) {
                    face = face.shift(CuboidDirection.North, 1);
                }
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1, this.z1, face.getUpperX(), this.y2, this.z2);
            case South:
                while (face.containsOnly(Material.AIR) && face.getUpperX() < this.getUpperX()) {
                    face = face.shift(CuboidDirection.South, 1);
                }
                return new Cuboid(this.regionID, this.worldName, face.getLowerX(), this.y1, this.z1, this.x2, this.y2, this.z2);
            case East:
                while (face.containsOnly(Material.AIR) && face.getLowerZ() > this.getLowerZ()) {
                    face = face.shift(CuboidDirection.East, 1);
                }
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1, this.z1, this.x2, this.y2, face.getUpperZ());
            case West:
                while (face.containsOnly(Material.AIR) && face.getUpperZ() < this.getUpperZ()) {
                    face = face.shift(CuboidDirection.West, 1);
                }
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1, face.getLowerZ(), this.x2, this.y2, this.z2);
            default:
                throw new IllegalArgumentException("Invalid direction " + dir);
        }
    }
    ...
    public Cuboid getFace(CuboidDirection dir) {
        switch (dir) {
            case Down:
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1, this.z1, this.x2, this.y1, this.z2);
            case Up:
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y2, this.z1, this.x2, this.y2, this.z2);
            case North:
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1, this.z1, this.x1, this.y2, this.z2);
            case South:
                return new Cuboid(this.regionID, this.worldName, this.x2, this.y1, this.z1, this.x2, this.y2, this.z2);
            case East:
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1, this.z1, this.x2, this.y2, this.z1);
            case West:
                return new Cuboid(this.regionID, this.worldName, this.x1, this.y1, this.z2, this.x2, this.y2, this.z2);
            default:
                throw new IllegalArgumentException("Invalid direction " + dir);
        }
    }
...
//     */
    ...
    public Cuboid getBoundingCuboid(Cuboid other) {
        ...
        return new Cuboid(this.regionID, this.worldName, xMin, yMin, zMin, xMax, yMax, zMax);
    }
    ...
    public boolean hasPlayerInside(Player player){
        ...
        if(x1 <= loc.getX() && x2 >= loc.getX() && y1 <= loc.getY() && y2 >= loc.getY() && z1 <= loc.getZ() && z2 >= loc.getX()){
            return true;
        }
        else{
            return false;
